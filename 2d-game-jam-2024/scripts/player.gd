...
@onready var pinky: Sprite2D = $Sprite2D/Pinky
...
func _process(delta: float) -> void:
	...
	sprite.rotate(PI/2)
	pinky.visible = GlobalVariables.debug
...
func spawn_in():
	...
	look_at(target)
	if randi_range(0, 100) == 0: # cheese
		print("cheese")
		pinky.texture = load("res://sprites/planets/cheese.png")
		pinky.scale = Vector2(0.04, 0.04)
	pinky.global_position = CENTER + offset # Placeholder to visualize offset
	rotate(PI/2)
