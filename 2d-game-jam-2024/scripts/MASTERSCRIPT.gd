...
func generate_planet():
	...
	planet_scale = planet.get_node("PlanetHitbox/CollisionShape2D").scale.x * speed_factor * scale_variance
	player.sideways_thrust *= (speed_factor - 1) * 5 + 1
	planet.scale *= scale_variance
	planet.starting_gravity *= min(planet_scale, 5)
...
func _process(delta: float) -> void:
	if menu == false && !game_is_over:
		...
		if !passed_periapsis:
			...
			planet.gravity_strength = planet.starting_gravity * 0.8
