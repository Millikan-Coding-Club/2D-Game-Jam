...
var planet_sprites_copy: Array[CompressedTexture2D] = []
@export var basic_planet_sprites: Array[CompressedTexture2D] = []
...
# TODO:
# - Scale planets and their stuff
# - Make planet chances weighted
# - 
func generate_planet():
	if randf() < cool_planet_chance: # Cool planets 😎
		if planet_sprites_copy.is_empty():
			planet_sprites_copy = planet_sprites.duplicate()
		var rand = randi_range(0, planet_sprites_copy.size() - 1)
		$Planet/Sprite2D.texture = planet_sprites_copy.pop_at(rand)
		$Planet/Sprite2D.self_modulate = Color(1, 1, 1)
	else: # Basic planets
		$Planet/Sprite2D.texture = basic_planet_sprites.pick_random()
		$Planet/Sprite2D.self_modulate = Color.from_hsv(randf(), 0.25, 1)
	$Planet/Rings.hide()
	$Planet/Clouds.hide()
	...
	if randf() < overlay_chance: # Clouds
		$Planet/Clouds.show()
		$Planet/Clouds.self_modulate = Color.from_hsv(randf(), 0.25, 1)
