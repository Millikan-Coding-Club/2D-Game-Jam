...
# TODO:
# - Finish adding all cool planets
...
func generate_planet():
	...
	if randf() < cool_planet_chance: 
		...
		#planet.get_node("Sprite2D").self_modulate = Color(1, 1, 1)
	else: # Basic planets
	...
	# Set planet values
	planet.get_node("Area2D/CollisionShape2D").scale = 100 * planet.get_node("PlanetHitbox/CollisionShape2D").scale
	#planet.get_node("Area2D").gravity_point_unit_distance *= planet.get_node("PlanetHitbox/CollisionShape2D").scale.x
